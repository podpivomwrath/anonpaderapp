...
DAILY_QUEST_COUNT = 3
# Патч 50: Метка Хранителя — 5 заданий вместо 3. Награда за КАЖДОЕ — по
# обычным правилам (daily_reward), для стрика по-прежнему достаточно одного.
# При истечении премиума лишние задания просто не назначаются НА СЛЕДУЮЩИЙ
# день (см. services/daily_service.py::_assign_new_dailies) — уже назначенные
# сегодня не отбираются, их можно доделать.
DAILY_QUEST_COUNT_PREMIUM = 5
