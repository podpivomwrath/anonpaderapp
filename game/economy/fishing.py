...
def roll_bite(rng: random.Random) -> tuple[float, float] | None:
    """Розыгрыш поклёвки: (секунд ожидания, бонус к доле веса) либо None —
    заброс вообще без поклёвки.

    Длинное ожидание намеренно СВЯЗАНО с бонусом к весу: пауза перестаёт быть
    пустым временем и становится ожиданием чего-то крупного.
    """
    if rng.random() < fc.EMPTY_CAST_CHANCE:
        return None
    roll = rng.random()
    cumulative = 0.0
