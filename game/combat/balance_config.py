...
CC_CHILL_WINDOW_TURNS = 1       # сколько ходов после контроля цель считается «по холоду» (бить нужно сразу следующим ходом)
...
# Патч 51, ч.4: групповой PvE — Страж с повышенной пассивной агрессией
# притягивает удары мобов чаще при случайном (не форсированном Провокацией)
# выборе цели. Вес — множитель к шансу быть выбранным относительно
# небазового участника (вес 1.0); Провокация (taunted_by) всё ещё форсит
# жёстко и это правило не затрагивает.
GROUP_PVE_GUARDIAN_AGGRO_WEIGHT = 2.0
...
# Ограничения по категориям в пресете НЕТ: игрок собирает любые баффы своего
# пула. Прежнее требование «хотя бы один из обороны или контроля» снято.
...
SUBCLASS_RESET_COST_DONATE = 50     # сброс только подкласса; TODO: калибровка
STATS_RESET_COST_FARM = 5000        # сброс характеристик; TODO: калибровка
# Альфа-тест: переделка персонажа у Хранителя бесплатна и без ограничений.
# Выключить флаг - и списание по константам выше включится само, код
# вызывающей стороны трогать не нужно.
ALPHA_FREE_RESPEC = True
...
SHADOW_BLADE_EXECUTE_HP_THRESHOLD = 0.30   # Казнь: ниже этой доли HP цели урон удваивается
SHADOW_BLADE_EXECUTE_LOW_HP_MULT = 2.0     # ...во сколько раз
...
DARK_MYSTIC_DRAIN_CONTROLLED_MULT = 1.40   # Иссушение: множитель урона по цели под контролем
...
DARK_MYSTIC_SELF_DENIAL_EXTRA_HP = 0.10     # Самоотречение: доп. плата % МАКСИМУМА HP
DARK_MYSTIC_SELF_DENIAL_BONUS = 0.50        # ...и насколько сильнее эффект
DARK_MYSTIC_SELF_DENIAL_MIN_HP = 0.50       # ...и ниже какой доли HP он больше не платит
# По тексту патча трата добровольная («МОЖНО потратить»), но бафф пассивный и
# платит на каждом касте. Плата от ТЕКУЩЕГО HP дешевела вместе со здоровьем и
# делала размен выгодным всегда; плата от максимума без порога, наоборот,
# загоняла мистика в петлю самоубийства. Порог возвращает смысл: пока здоров -
# доплачивает и бьёт сильнее, просел - перестаёт.
...
POISONER_BURST_MAX_TICKS = 2                # Токсический выброс: сколько ходов яда переносит разом
# Выброс снимает ровно столько, сколько переносит: остаток яда продолжает
# тикать. Без потолка он сваливал в один удар всю оставшуюся длительность
# (до 542 урона) и выносил дуэли, а без переноса вообще - уничтожал больше,
# чем наносил, и применять его не имело смысла никогда.
