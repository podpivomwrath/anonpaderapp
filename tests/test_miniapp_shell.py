...
def test_centred_columns_keep_a_definite_width() -> None:
    ...
    Проверяются блоки, где auto стоит с ОБЕИХ сторон, то есть настоящее
    центрирование. Один только margin-left: auto - обычный приём «прижать
    к концу ряда» (так стоит галочка в листе выбора рамки), растягивание он
    не отменяет, и требовать от него ширину незачем. Первая версия теста
    этого не различала и ругалась на здоровый код.
    """
    # Комментарии вырезаются ДО проверки. В том самом блоке написано
    # «width: 100% обязателен», и поиск по тексту находил объяснение вместо
    # самого правила: тест оставался зелёным с убранным width. В этом
    # проекте такое случалось уже трижды (патчи 73, 82) - проверять надо
    # механизм, а не слова рядом с ним.
    css = re.sub(r"/\*.*?\*/", "", (MINIAPP / "src" / "index.css").read_text(encoding="utf-8"),
                 flags=re.DOTALL)
    for match in re.finditer(r"\{([^{}]*margin-left:\s*auto[^{}]*)\}", css):
        block = match.group(1)
        if not re.search(r"margin-right:\s*auto", block):
            continue
