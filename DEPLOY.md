...
## Обновление с проверкой и восстановление рейдов

`scripts/deploy-release.sh` собирает образы при работающем боте, создаёт и
проверяет сжатый дамп БД, останавливает бот, применяет миграции отдельным
контейнером и запускает новую версию. При ошибке после остановки возвращает
предыдущие образы; БД автоматически не откатывается. Для этой версии миграция
0025 только добавляет таблицу и совместима со старым приложением.

Скрипту нужны права записи в `/opt/backups` (либо задайте `BACKUP_DIR`).
Дампы `pre_release_*` автоматически не удаляются. PostgreSQL/Redis и их тома
при выкладке не пересоздаются. После запуска проверяются `/health` и ревизия
Alembic. Снаружи дополнительно проверьте мини-приложение и API через Caddy.

Активные бои по-прежнему находятся в памяти. Начиная с версии с миграцией
0025, при техническом прерывании рейда ключ автоматически возвращается
лидеру один раз. Ранее выданные награды сохраняются. Старые активные рейды
версии 0024 не имеют журнала запусков и автоматически восстановлены быть
не могут: планируйте переход в период без активных рейдов.
